...
class BBCodeParser extends JBBCode\Parser {
	...
	public function __construct() {
		...
		$builder = new JBBCode\CodeDefinitionBuilder('file', '<div class="file-{option}"><img src="{param}"/></div>');
		$builder->setUseOption(true);
		$this->addCodeDefinition($builder->build());
	...
	public function parse($str) {
		$str = str_replace("\t", "    ", $str);
		$regex = "#(.*?)\r\n#";
		$regex2 = "#\[p\](.*?)(\[/?(h1|h2|h3|h4|h5|h6|li|ul|div|pre|code|sectioncode|legend|quote|becareful|file|info|left|leftedcode|center|justify|section2|section3|p|ol|list|\*|youtube|video|table|td|tr|th)(=.*?)?\])(.*?)\[/p\]\r\n#";
//		$regex3 = "#(\[code\])(.*?)(\[/?p\])(.*?)(\[/code\])#";
		$str = preg_replace($regex, "[p]$1[/p]\r\n", $str . "\r\n");
		$str = preg_replace($regex2, "$1$2$5\r\n", $str);
//		$str = preg_replace($regex3, "$1$2$4$5", $str);
		$str = html_escape($str);
		$newstr = '';
		$min = 0;
		$max = 0;
